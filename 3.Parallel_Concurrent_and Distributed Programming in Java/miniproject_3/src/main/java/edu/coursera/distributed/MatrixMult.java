...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int rowsPerRank = (nRows + numRanks - 1) / numRanks;

        final int startRow = myRank * rowsPerRank;
        final int endRow = Math.min(nRows, (myRank + 1) * rowsPerRank);
        ...
        for (int i = startRow; i < c.getNRows(); i++) {
        ...
        if(myRank==0){
            ...
            for(int i=1; i<numRanks; i++){
                final int start = i * rowsPerRank;
                final int end = Math.min((i + 1) * rowsPerRank, nRows);

                final int rowOffset = start * c.getNCols();
                final int rowCount = (end - start) * c.getNCols();

                requests[i-1] = mpi.MPI_Irecv(c.getValues(), rowOffset, rowCount, i, i, mpi.MPI_COMM_WORLD);
            }
